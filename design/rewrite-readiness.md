...
- [x] More transcript fixtures: a subagent/sidechain session, a post-compaction
      ...
      `tool_result` content (an array of non-text blocks renders as an empty
      body, not `(no content)` - that string only appears when `content` is
      neither a string nor an array), and a >1 MB session for the perf budget
      (generated by `just fixture-large`, not committed).
